...
// AAS-Artillery/functions/fn_initClientArtillery.sqf

if (!hasInterface) exitWith {};

diag_log "[AAS-Artillery] Initializing Artillery Client Module...";

if (isNil "AAS_Loaded_Modules") then { AAS_Loaded_Modules = []; };
if (isNil "AAS_Menu_Registry") then { AAS_Menu_Registry = []; };

AAS_Loaded_Modules pushBackUnique "ARTILLERY";
...
aas_artillery_fnc_startTargeting = {
    ...
    [_caller, _strikeType] spawn {
        ...
        [] spawn { sleep 0.5; playSound "aas_art_laserdesignate"; };
        ...
        private _hudY = missionNamespace getVariable ["AAS_ART_HUD_Height", 0.35];
        private _hudSize = missionNamespace getVariable ["AAS_ART_HUD_Size", 1.0];
        private _dangerRadius = missionNamespace getVariable ["AAS_ART_DangerClose_Radius", 150];
        ...
        private _refreshTargetingUI = {
            ...
            if (!isNil "AAS_ART_EVH_Scroll") then { _display displayRemoveEventHandler ["MouseZChanged", AAS_ART_EVH_Scroll]; AAS_ART_EVH_Scroll = nil; };
            if (!isNil "AAS_ART_EVH_Click") then { _display displayRemoveEventHandler ["MouseButtonUp", AAS_ART_EVH_Click]; AAS_ART_EVH_Click = nil; };
            if (!isNil "AAS_ART_EVH_Key") then { _display displayRemoveEventHandler ["KeyDown", AAS_ART_EVH_Key]; AAS_ART_EVH_Key = nil; };
            ...
            if (_uiState == "LASER") then {
                ...
                AAS_ART_EVH_Scroll = _display displayAddEventHandler ["MouseZChanged", {
                    params ["", "_scroll"];
                ...
                AAS_ART_EVH_Click = _display displayAddEventHandler ["MouseButtonUp", { 
                    params ["", "_button"];
                    if (cameraView == "GUNNER") exitWith {false};
                ...
                AAS_ART_EVH_Key = _display displayAddEventHandler ["KeyDown", {
                    params ["", "_key"];
        ...
        [_pagerDisplay] spawn {
            ...
            if (!isNull _mainDisplay) then {
                if (!isNil "AAS_ART_EVH_Scroll") then { _mainDisplay displayRemoveEventHandler ["MouseZChanged", AAS_ART_EVH_Scroll]; };
                if (!isNil "AAS_ART_EVH_Click") then { _mainDisplay displayRemoveEventHandler ["MouseButtonUp", AAS_ART_EVH_Click]; };
                if (!isNil "AAS_ART_EVH_Key") then { _mainDisplay displayRemoveEventHandler ["KeyDown", AAS_ART_EVH_Key]; };
            };
        ...
        while {time < _timeout && !_locked && (_caller getVariable ["AAS_TargetMode", "LASER"] != "CANCEL")} do {
            ...
            if (_mode == "MAP") then {
                if (isNull (findDisplay 8990)) then {
                    ...
                    createDialog "AAS_ART_Tablet_Dialog";
        ...
        if (_locked) then {
            systemChat "HQ: Got a lock on the target, sending grid to FDC.";
            ...
            [] spawn { sleep 0.2; playSound "aas_art_targetexpired"; };
        };
    };
};
...
aas_artillery_fnc_refreshArtillery = {
    ...
    switch (_econPreset) do {
        case 0: { _baseCost = parseNumber (missionNamespace getVariable ["AAS_ART_Cost_Base_Custom", "0"]) };
        case 1: { _baseCost = parseNumber (missionNamespace getVariable ["AAS_ART_Cost_Base_Antistasi", "1000"]) };
        case 2: { 
            private _s = parseNumber (missionNamespace getVariable ["AAS_ART_Cost_Base_KPLib_S", "50"]);
            private _a = parseNumber (missionNamespace getVariable ["AAS_ART_Cost_Base_KPLib_A", "100"]);
            private _f = parseNumber (missionNamespace getVariable ["AAS_ART_Cost_Base_KPLib_F", "50"]);
            _kpLibString = format ["%1/%2/%3", _s, _a, _f];
        };
        case 3: { _baseCost = parseNumber (missionNamespace getVariable ["AAS_ART_Cost_Base_Overthrow", "3000"]) };
        case 4: { _baseCost = parseNumber (missionNamespace getVariable ["AAS_ART_Cost_Base_Warlords", "400"]) };
        case 5: { _baseCost = parseNumber (missionNamespace getVariable ["AAS_ART_Cost_Base_DUWS", "10"]) };
        case 6: { _baseCost = parseNumber (missionNamespace getVariable ["AAS_ART_Cost_Base_Antistasi", "1000"]) }; 
    };

    // --- UPGRADED HELPER: BUILDS NESTED SUBMENUS ---
    private _processCategory = {
        params ["_categoryName", "_munitionsArray"];
        private _categorySubMenu = []; // We will build the strikes into this folder
        
        {
            _x params ["_name", "_multVar", "_cdVar", "_lastUseVar", "_id"];
            private _cd = parseNumber (missionNamespace getVariable [_cdVar, "600"]);
            private _last = missionNamespace getVariable [_lastUseVar, -99999];

            if (serverTime >= (_last + _cd)) then {
                private _actionName = _name; // No longer pasting the prefix onto the string
        ...
        // Clean UI: Only add the Category to the main Artillery menu if there are available strikes inside it
        if (count _categorySubMenu > 0) then {
            _artilleryMenu pushBack [_categoryName, _categorySubMenu];
        };
    };
    ...
    [
        "82mm Mortar", 
        [
            ["HE x7", "AAS_ART_Mult_82mm_HEX7", "AAS_ART_Cooldown_Global", "AAS_ART_LastUse_82mm_HEX7", "Arty82mm_HEX7"],
            ["HE x14", "AAS_ART_Mult_82mm_HEX14", "AAS_ART_Cooldown_Global", "AAS_ART_LastUse_82mm_HEX14", "Arty82mm_HEX14"],
            ["Smoke", "AAS_ART_Mult_82mm_Smoke", "AAS_ART_Cooldown_Global", "AAS_ART_LastUse_82mm_Smoke", "Arty82mm_Smoke"]
        ]
    ] call _processCategory;
    ...
    [
        "155mm Howitzer", 
        [
            ["HE x5", "AAS_ART_Mult_155mm_HEX5", "AAS_ART_Cooldown_Global", "AAS_ART_LastUse_155mm_HEX5", "Arty155mm_HEX5"],
            ["HE x10", "AAS_ART_Mult_155mm_HEX10", "AAS_ART_Cooldown_Global", "AAS_ART_LastUse_155mm_HEX10", "Arty155mm_HEX10"],
            ["DPICM", "AAS_ART_Mult_155mm_DPICM", "AAS_ART_Cooldown_Global", "AAS_ART_LastUse_155mm_DPICM", "Arty155mm_DPICM"],
            ["ADAM", "AAS_ART_Mult_155mm_ADAM", "AAS_ART_Cooldown_Global", "AAS_ART_LastUse_155mm_ADAM", "Arty155mm_ADAM"],
            ["RAAMS", "AAS_ART_Mult_155mm_RAAMS", "AAS_ART_Cooldown_Global", "AAS_ART_LastUse_155mm_RAAMS", "Arty155mm_RAAMS"]
        ]
    ] call _processCategory;

    // ==========================================
    // --- 3. MLRS ROCKET STRIKE ---
    // ==========================================
    [
        "MLRS Rocket Strike", 
        [
            ["HEDP x7", "AAS_ART_Mult_MLRS_HEDPX7", "AAS_ART_Cooldown_Global", "AAS_ART_LastUse_MLRS_HEDPX7", "ArtyMLRS_HEDPX7"],
            ["HEDP x14", "AAS_ART_Mult_MLRS_HEDPX14", "AAS_ART_Cooldown_Global", "AAS_ART_LastUse_MLRS_HEDPX14", "ArtyMLRS_HEDPX14"],
            ["White Phosphor", "AAS_ART_Mult_MLRS_WP", "AAS_ART_Cooldown_Global", "AAS_ART_LastUse_MLRS_WP", "ArtyMLRS_WP"]
        ]
    ] call _processCategory;

    // Inject the fully built, nested array into the master registry
    private _found = false;
    { 
        if ((_x select 0) == "ARTILLERY") exitWith { 
            _x set [1, _artilleryMenu]; 
            _found = true; 
        }; 
    } forEach AAS_Menu_Registry;
    
    if (!_found) then { 
        AAS_Menu_Registry pushBack ["ARTILLERY", _artilleryMenu]; 
    };
};
...
diag_log "[AAS-Artillery] SUCCESS: Artillery Module integrated into Tactical Pager Registry.";